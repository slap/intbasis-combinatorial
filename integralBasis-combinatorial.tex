...
\begin{abstract}
%In \cite{intbas}, we gave an algorithm for computing integral bases in function fields
%of plane curves which has  quasi-optimal  runtime \cite{M1}. Our approach is local-to-global in
%the sense that we first compute a local contribution to the normalization at each singularity of the
%curve, and then put these local contributions together. To find a local contribution at a given
%singularity in the first step, we rely on Puiseux expansions and Hensel lifting to find a contribution
%to the normalization at each branch of the singularity, and then merge these contributions using
%Chinese remaindering. In this paper, we present a combinatorial approach for merging which often
%performs better in practise, notably in cases where the curve has a large number of branches at
%one of its singularities.

% Changed to
In our previous work \cite{intbas}, we have developed an algorithm for computing integral bases of plane curves. Our approach is local-to-global in
the sense that we first compute a local contribution to the normalization at each singularity of the
curve, and then put these local contributions together. To find a local contribution at a given
singularity in the first step, we rely on Puiseux expansions and Hensel lifting to find a contribution
to the normalization at each branch of the singularity, and then merge these contributions using
Chinese remaindering.
In this paper, we develop a variant of our algorithm which, in practice, shows an enhanced performance. The main improvement over \cite{intbas} is a combinatorial approach to find the numerators of the integral basis elements. In contrast to the old algorithm, the new variant directly produces an integral basis, and does not require to transform the generators of the local contributions into triangular integral basis form. The new approach is particularly useful for curves with many branches at the singularity.
\end{abstract}
\maketitle
...
%In this paper, $A$ will be the coordinate ring
...
% Changed to
In this paper, $A$ will be the coordinate ring of an algebraic curve.  Our goal is to compute the integral closure $\bar A$ in terms of an integral basis.  We focus on the case where the curve is defined over a field $K$ of characteristic zero and is irreducible over $K$.
To fix to our notation, let $f\in K[X,Y]$ be an irreducible polynomial in two variables, let $C\subset\mathbb{A}^{2}(K)$ be the affine plane curve
defined by $f$, and let
%
\[
...
%In particular, $A$ is integral over $K[x]$, which implies that $\overline{A}$ coincides with the
...
% Changed to
In particular, $A$ is integral over $K[x]$. This implies $\overline{A}$ is equal to the integral closure of $K[x]$ in $K(C)$. Hence, $\overline{A}$ can be represented both as an $A$-module or as a module over $K[x] \cong K[X]$. As noted in \cite[Remark 2]{intbas}, $\overline{A}$ is a free $K[x]$-module of rank
\[
n:=\deg_{Y}(f)=[K(C):K(x)].
\]
...
% Removed - This part of the remark is not needed
%In fact, such a
...
In \cite{intbas}, we gave an algorithm to compute integral bases for $\overline{A}$ which is
local-to-global in the following sense: First, for each prime ideal $P$ of the singular locus $\Sing(A)$,
find a local contribution to the integral basis at $P$. Second, add up these local contributions.
In this paper, we are concerned with improving the first step.

%For simplicity of our presentation, we assume that
%$\Sing(A)$ consists of just one prime ideal $P$, and that this prime ideal defines a $K$-rational singularity
%of $C$ which is located at the origin\footnote{If there are more singularities or conjugated singularities,
%the techniques presented in \cite{intbas} allow us to reduce the problem to the case dealt with here.}.

% Changed to
For this step, after a linear change of coordinates, we can restrict to the problem of computing an integral basis for the normalization of the localization of $A$ at $x = 0$, assuming that the origin is the only singularity of the localization. \footnote{For a group of conjugated singularities,
by the techniques presented in \cite{intbas} it is enough to handle a single point of the conjugated group.}

In this setting, consider the decomposition
\begin{equation}
f=f_{0}\widetilde{f}=f_{0} f_{1}\cdots f_{r}
\label{equation:branches}%
\end{equation}
given by the Weierstrass preparation theorem (see \cite{Abhyankar}, \cite{JP}).
Here, $f_{0}\in K[[X]][Y]$ is a unit in $K[[X,Y]]$, and $f_{1}, \dots, f_{r}\in K[[X]][Y]$ are irreducible
Weierstrass polynomials to which we refer as the \emph{branches} of $f$ (over $K$, centered at
$P$). If $g=f$, $g= \widetilde{f}$, or $g$ is one of the branches $f_i$, and
$$B_g=K[[x]][y] =K[[X]][Y]/\langle g \rangle,$$
then it makes sense to speak of integral bases for $\overline{B_g}$ over $K[[x]]$ (see \cite{intbas}).

Integral bases for the $\overline{B_{f_i}}$ can be computed using Puiseux series and Hensel lifting
as discussed in \cite{intbas}. Merging the computed bases gives us an integral basis for
$\overline{B_{\widetilde{f}}}$. From this, we can read off an integral basis for $\overline{B_{{f}}}$
as described in \cite[Proposition 41]{intbas}. This basis is the desired local contribution to the
integral basis for $\overline{A}$ at $P$.

In our previous paper, the merging of the integral bases for the $\overline{B_{\widetilde{f_i}}}$
is done using Chinese remaindering. This can be inefficient if there is a large number of branches
of $f$ at $P$.
%Indeed, to find the cofactors required for the Chinese remainder step, we
%may need to develop the involved Puiseux expansions up to a high order, and the merging of the
%integral bases may require the computation of Gr\"obner bases of possibly complicated ideals.
%In contrast, the direct method introduced in this paper can handle all conjugacy classes
%simultaneously, without computing Gr\"obner bases.
%
% Changed to
Indeed, in the Chinese remainder step, multiplying the generators of the local integral bases by the cofactors requires that the branches (computed via Hensel lifting) are developed up to a higher order. Moreover, summing up the result may lead to a set of  generators which is not in triangular integral basis shape, and computing the triangular shape can be computationally expensive.

In contrast, the direct method introduced in this paper uses a combinatorial procedure to obtain an integral basis at the origin by
handling all the different conjugacy classes at once, and directly returns the generators in triangular shape.

On our way to explaining this, we first recall some formulae for valuations.


\subsection{Valuations}

Let $L\{\{X\}\}$ be the field of Puiseux series. A nonzero element $f \in L\{\{X\}\}$ can be uniquely written as
$$
f = \sum_{k = k_0}^\infty c_k X^{k / n},
$$
with $c_k \in L$ for all $k \ge k_0$, $c_{k_0} \neq 0$, and $n \in \N$. The \emph{valuation} of $f$ is
$\upsilon_X(f) = \frac{k_0}{n}$. The corresponding \emph{valuation ring}
...
Note that by Lemma \ref{lemma:intA} and the choice of $\widetilde{p}_d$, the inequality is in fact an equality since
$\widetilde{p}_d$ maximizes the valuation over $L\{\{X\}\}$.
%Note that $K[X]$ is included in all the rings $R$ in the above list, hence the reverse inequalities are trivial, and the result follows.
\end{proof}
